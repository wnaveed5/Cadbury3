...
function SortableTotalsField({ field, onRemove, onLabelChange, onContentChange, onValueChange, showDummyData = true, getNetSuiteVariable }) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    >
      ...
      {field.isCalculated ? (
        ...
        showDummyData ? (
          ...
          >
            {getNetSuiteVariable ? getNetSuiteVariable(field.id, 'totals') : `\${record.${field.id}}`}
          </span>
        )
      )}
    </div>
