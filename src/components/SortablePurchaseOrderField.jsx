...
function SortablePurchaseOrderField({ field, onRemove, onLabelChange, onContentChange, section, showDummyData = true, getNetSuiteVariable }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="po-field-item" data-field={field.id}>
      {/* Debug logging for title fields */}
              {field.isTitle && console.log('Rendering title field:', {
        id: field.id,
        label: field.label,
      ...
      <div className="po-field-content">
        ...
        {!field.isTitle && (
          showDummyData ? (
            ...
            >
              {getNetSuiteVariable ? getNetSuiteVariable(field.id, 'purchaseOrder') : `\${record.${field.id}}`}
            </span>
          )
        )}
      </div>
